%加载并设置宏包
%广西大学物理学院2021级硕士王信哲制作
\ctexset{
	bibname = 参考文献,
	contentsname=\heiti\zihao{3} 目\ 录,
	section={
		name={第,章},number=\chinese{section},
		format=\heiti\bfseries\centering\zihao{3}\linespread{1.25},
		beforeskip=20pt,%18ex plus 0.2ex minus .2ex,
		afterskip=20pt%18ex plus 0.2ex minus .2ex
	},
	subsection={
		format=\heiti\bfseries\raggedright\zihao{4}\linespread{1.25},
		beforeskip=8.75pt,%18ex plus 0.1ex minus .1ex,
		afterskip=8.75pt%18ex plus 0.1ex minus .1ex
	},
	subsubsection={
		format=\heiti\bfseries\raggedright\zihao{-4}\linespread{1.25},
		beforeskip=0pt,
		afterskip=0pt
	},
	appendix/name=附录
}
%\usepackage{ctex}%設置中文

\usepackage{tocloft}
\renewcommand{\cfttoctitlefont}{\hfill} % 将目录标题设置为三号字、黑体
\renewcommand{\cftaftertoctitle}{\hfill} % 将目录标题居中
\renewcommand{\cftsecleader}{\cftdotfill{\cftdotsep}}%让一级标题的章节和页面之间也有点线连接
\renewcommand{\cftsecfont}{\normalfont}%取消section字体在目录中加粗
\renewcommand{\cftsecpagefont}{\normalfont}%取消section在目录中对应的页码加粗
\setlength{\cftbeforesecskip}{0pt}
\renewcommand{\cftdotsep}{1}

% --- 设置中文字体 ---
% 尝试寻找系统中的“宋体”，如果找不到则使用当前目录下的 FZSSK.TTF。\songti命令在下文定义。
\IfFontExistsTF{宋体}{
	\setCJKmainfont{宋体}
}{
	\setCJKmainfont{FZSSK.TTF}[
	Path=./字体/,
	% 如果该字体有对应的粗体/斜体文件，可以在这里指定，否则系统会通过算法伪粗体
	% BoldFont = FZSSK_Bold.TTF, 
	]
}
% --- 设置英文字体 ---
% 尝试寻找系统中的“Times New Roman”，如果找不到则使用当前目录下的 TIMES.TTF
\IfFontExistsTF{Times New Roman}{
	\setmainfont{Times New Roman}
}{
	\setmainfont{TIMES.TTF}[
	Path=./字体/,
	% 注意：Times New Roman 通常有四个文件（标准、粗体、斜体、粗斜体）
	 BoldFont=TIMESBD.TTF,
	 ItalicFont=TIMESI.TTF,
	 BoldItalicFont=TIMESBI.TTF
	]
}
% --- 3. 设置仿宋字体 (\fangsong) ---
% ctex 的 \fangsong 命令内部调用的是 zhfs 字体族
\IfFontExistsTF{仿宋}{
	\setCJKfamilyfont{zhfs}{仿宋}
}{
	\IfFileExists{./字体/FS.TTF}{
		% 如果系统没仿宋，但当前目录有 FS.TTF，则将其定义为 zhfs 族
		\setCJKfamilyfont{zhfs}{FS.TTF}[Path=./字体/]
	}{
		% 如果两个都没有，Overleaf 通常自带 FandolFangsong
		\IfFontExistsTF{FandolFangsong-Regular.otf}{
			\setCJKfamilyfont{zhfs}{FandolFangsong-Regular.otf}
		}{
			\typeout{Warning: Fangsong font not found.}
		}
	}
}
% 重新定义 \fangsong 命令，确保它指向我们刚才定义的 zhfs 族
\renewcommand*{\fangsong}{\CJKfamily{zhfs}}

% --- 4. 设置隶书字体 (\lishu) ---
% ctex 的 \lishu 命令内部调用的是 zhli 字体族
\IfFontExistsTF{隶书}{
    \setCJKfamilyfont{zhli}{隶书}
}{
    \IfFileExists{./字体/SIMLI.TTF}{
        % 如果系统没有隶书，但 ./字体/ 目录下有 SIMLI.TTF，则将其定义为 zhli 族
        \setCJKfamilyfont{zhli}{SIMLI.TTF}[Path=./字体/]
    }{
        % 如果两个都没有，由于 Fandol 没提供隶书，这里通常回退到楷体或报警告
        \IfFontExistsTF{FandolKai-Regular.otf}{
            \typeout{Warning: Lishu font not found, falling back to FandolKai.}
            \setCJKfamilyfont{zhli}{FandolKai-Regular.otf}
        }{
            \typeout{Warning: Lishu font not found.}
        }
    }
}
% 重新定义 \lishu 命令，确保指向刚才定义的 zhli 族
% \providecommand 是为了防止某些极简配置下命令未定义，\renewcommand 则是覆盖 ctex 默认
\makeatletter
\DeclareRobustCommand*{\lishu}{\CJKfamily{zhli}}
\makeatother

% --- 5. 设置黑体字体 (\heiti) ---
\IfFontExistsTF{黑体}{
    \setCJKfamilyfont{zhhei}{黑体}
}{
    \IfFileExists{./字体/SIMHEI.TTF}{
        \setCJKfamilyfont{zhhei}{SIMHEI.TTF}[Path=./字体/]
    }{
        % 如果都没有，强制使用 Linux 自带的 Fandol 黑体作为保底
        \IfFontExistsTF{FandolHei-Regular.otf}{
            \setCJKfamilyfont{zhhei}{FandolHei-Regular.otf}
        }{
            \typeout{Warning: Heiti not found!}
        }
    }
}
% 统一 \heiti 命令
\makeatletter
\DeclareRobustCommand*{\heiti}{\CJKfamily{zhhei}}
\makeatother

% --- 6. 设置宋体命令 (\songti) ---
% ctex 的 \songti 命令内部调用的是 zhsong 字体族
\IfFontExistsTF{宋体}{
    \setCJKfamilyfont{zhsong}{宋体}
}{
    \IfFileExists{./字体/FZSSK.TTF}{
        % 如果系统没有宋体，但 ./字体/ 目录下有 FZSSK.TTF，则将其定义为 zhsong 族
        \setCJKfamilyfont{zhsong}{FZSSK.TTF}[Path=./字体/]
    }{
        % 如果两个都没有，回退到 Linux/Overleaf 自带的 FandolSong
        \IfFontExistsTF{FandolSong-Regular.otf}{
            \setCJKfamilyfont{zhsong}{FandolSong-Regular.otf}
        }{
            \typeout{Warning: Songti font not found.}
        }
    }
}
% 重新定义 \songti 命令，确保它指向我们刚才定义的 zhsong 族
\makeatletter
\DeclareRobustCommand*{\songti}{\CJKfamily{zhsong}}
\makeatother


\usepackage{amsmath,amssymb,amsthm}
\let\hbaroriginal\hbar
\AtBeginDocument{\let\hbar\hbaroriginal}
%\usepackage{unicode-math}
%\setmathfont{Latin Modern Math}

\usepackage{geometry}
\geometry{papersize={21cm,29.7cm}}
\geometry{left=2.8cm,right=2.5cm,top=2.8cm,bottom=2.2cm}
\usepackage{mathrsfs}
\usepackage{wasysym}
\usepackage{braket}
\usepackage{indentfirst}
\usepackage{cases}
\usepackage{bm}
\usepackage{graphicx}
\usepackage{subcaption}
\usepackage{longtable,booktabs}
\usepackage{color}
\usepackage[colorlinks,linkcolor=blue,anchorcolor=blue,citecolor=blue]{hyperref}%#006eb2
%\hypersetup{
	%	colorlinks=true,
	%	linkcolor=[rgb]{0,0.43,0.70},
	%	citecolor=[rgb]{0,0.43,0.70},
	%	filecolor=[rgb]{0,0.43,0.70},
	%	urlcolor=[rgb]{0,0.43,0.70}
	%}
\usepackage{cleveref}   %可以调用 \Cref{fig:xxx}或者 \ref{fig:xxx}
\usepackage{pdfpages}
\usepackage{float}
\usepackage{booktabs}
\usepackage{fancyhdr}%設置頁眉

\usepackage{setspace}%设置行间距
\setstretch{1.25}

\usepackage[nottoc]{tocbibind}%[nottoc,numbib]
%\usepackage{gbt7714}
\usepackage{gbt7714-gxu}
\bibliographystyle{gxu}
\usepackage{natbib}
\setlength{\bibsep}{0pt}
%\bibliographystyle{gbt7714-numerical}

\usepackage{appendix}


\usepackage{booktabs}
\usepackage{lipsum} % 用于生成随机文本
\graphicspath{{图/}{./}}
\usepackage[justification=centering]{caption}
\usepackage{bicaption}
\captionsetup[figure][bi]{labelfont=bf, textfont=bf}
\captionsetup[figure][bi-second]{name=Figure,labelfont=bf,textfont=bf}
\captionsetup[table][bi]{labelfont=bf, textfont=bf}
\captionsetup[table][bi-second]{name=Table,labelfont=bf,textfont=bf}

%设置图表编号格式为{章节号}-{章节内序号}
\usepackage{chngcntr}
\counterwithin{figure}{section}
\counterwithin{table}{section}
\renewcommand{\thefigure}{\thesection-\arabic{figure}}
\renewcommand{\thetable}{\thesection-\arabic{table}}

\usepackage{pdfpages}

% 定义中文摘要环境
\newenvironment{cnabstract}{
	\par\small
	\noindent\mbox{}\hfill{\bfseries \cnabstractname}\hfill\mbox{}
	\par\vskip 2.5ex
}{\par\vskip 2.5ex}

% 定义英文摘要环境
\newenvironment{enabstract}{
	\par\small
	\noindent\mbox{}\hfill{\bfseries \enabstractname}\hfill\mbox{}
	\par\vskip 2.5ex
}{\par\vskip 2.5ex}

\newcommand{\cnabstractname}{\heiti \fontsize{16pt}{20pt}\textbf{摘\ 要}} % 中文摘要标题
\newcommand{\enabstractname}{\fontsize{16pt}{20pt}\textbf{ABSTRACT}} % 英文摘要标题


